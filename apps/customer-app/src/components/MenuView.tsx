...
interface MenuViewProps {
  tenantId: string;
  /** When set, shows "Table X" in header (table ordering flow) */
  tableNumber?: string | null;
}

export function MenuView({ tenantId, tableNumber }: MenuViewProps) {
      ...
      <div className="flex items-center justify-center min-h-screen bg-stone-50 p-6">
        <div className="text-center max-w-md">
          ...
          <Link
            ...
            className="inline-flex gap-2 px-4 py-2 rounded-xl text-white font-medium text-sm"
    ...
    return (
      <div className="min-h-screen min-h-dvh bg-stone-50 flex flex-col items-center justify-center p-6">
        ...
        <div className="text-center max-w-sm">
          ...
          <Link
            href={`/${tenantId}/dashboard`}
            className="inline-flex gap-2 px-5 py-2.5 rounded-xl text-white font-medium text-sm transition-colors"
            style={{ backgroundColor: 'var(--brand-primary)' }}
          >
      ...
      <div className="sticky top-0 z-20 bg-white/90 backdrop-blur-sm border-b border-stone-100 pt-[env(safe-area-inset-top,0px)] shadow-soft">
        <div className="flex items-center justify-between px-4 py-4">
          ...
          <h1 className="text-xl font-semibold text-stone-900">
            {tableNumber ? `Table ${tableNumber} · Menu` : 'Menu'}
          </h1>
          <button
